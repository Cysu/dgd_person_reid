...
trainval=${MODELS_DIR}/ft_jstl_dgd_split_${split_index}_${model}_trainval.prototxt
solver=${MODELS_DIR}/ft_jstl_dgd_split_${split_index}_${model}_solver.prototxt
sed -e "s/\${split_index}/${split_index}/g" \
    ${MODELS_DIR}/ft_jstl_dgd_${model}_trainval.prototxt > ${trainval}
sed -e "s/\${split_index}/${split_index}/g" \
    ${MODELS_DIR}/ft_jstl_dgd_${model}_solver.prototxt > ${solver}

# Fine-tuning.
GLOG_logtostderr=1 mpirun -n 2 ${CAFFE_DIR}/build/tools/caffe train \
    -solver ${solver} -weights ${weights} -gpu 0,1 \
    2>&1 | tee ${LOGS_DIR}/ft_jstl_dgd_split_${split_index}_${model}.log
